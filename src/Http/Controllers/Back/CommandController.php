...
class CommandController extends BaseController
{
    ...
    {
        $commands = config('administrable-shop.models.command')::online()->latest()->get();

        return view('administrable-shop::' . Str::lower(config('administrable.back_namespace')) . '.commands.index', compact('commands'));
    }

    public function create()
    {
        $command  = new (config('administrable-shop.models.command'));

        $products = config('administrable-shop.models.product')::with('media')->principal()->latest()->get()->each->append(['gallery']);
        $clients  = User::all();
        $states   = $command->getStates();
        $delivers = config('administrable-shop.models.deliver')::with('areas')->get();

        return view('administrable-shop::' . Str::lower(config('administrable.back_namespace')) . '.commands.create', compact(
            'command',
            'products',
            'clients',
            'states',
    ...
    public function store(Request $request)
    {
        ...
        $deliver  = config('administrable-shop.models.deliver')::where('id', $request->input('deliver.id'))->with([
        ...
        $products = array_map(function ($item) {
            return [
                ...
                'name'     => null,
                'model'    => config('administrable-shop.models.product'),
         ...
         */
        $command = config('administrable-shop.models.command')::create($data);
    ...
    {
        $command = config('administrable-shop.models.command')::where('id', $id)->firstOrFail();

        $command->load('client');

        $products = config('administrable-shop.models.product')::principal()->latest()->get();
        ...
        return view('administrable-shop::' . Str::lower(config('administrable.back_namespace')) . '.commands.edit', compact(
            'command',
            'products',
            'clients',
            'delivers',
    ...
    public function addProduct(int $id, Request $request)
    {
        ...
        $product = config('administrable-shop.models.product')::findOrFail($request->get('rowId'));
    ...
    {
        $orders = config('administrable-shop.models.order')::with('command')->get();
        ...
        $most_sales_products = config('administrable-shop.models.product')::mostSales();
