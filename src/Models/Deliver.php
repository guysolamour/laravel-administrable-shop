...
class Deliver extends BaseModel implements HasMedia
{
    ...
    {
        return $this->belongsToMany(config('administrable-shop.models.coveragearea'), 'shop_deliver_area_prices', 'deliver_id', 'coveragearea_id')
                ->withPivot('price')
                ->using(config('administrable-shop.models.deliverprice'))
                ;
    }
