...
trait AccessorsTrait
{
    ...
    public function getDeliversCoverageAreasAttribute(): array
    {
        ...
        $coverage_areas = array_map(function ($item) {
            $area = config('administrable-shop.models.coveragearea')::find($item['area_id']);
            ...
            return [
                'area'     => $area,
                'delivers' => config('administrable-shop.models.deliver')::whereIn('id', $item['delivers_ids'])->with('areas')->get(),
            ];
        }, $coverage_areas);
