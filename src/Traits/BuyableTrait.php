...
trait BuyableTrait
{
    ...
    {
        return $this->belongsToMany(config('administrable-shop.models.attribute'), 'shop_products_attributes');
    }


    public function reviews()
    {
        return $this->hasMany(config('administrable-shop.models.review'))->where('approved', true);
    }


    public function disapprovedReviews()
    {
        return $this->hasMany(config('administrable-shop.models.review'), 'product_id')->where('approved', false);
    }


    public function attribute()
    {
        return $this->belongsTo(config('administrable-shop.models.attribute'));
    }
    ...
    {
        return $this->hasMany(config('administrable-shop.models.product'), 'parent_id');
    }


    public function parent()
    {
        return $this->belongsTo(config('administrable-shop.models.product'), 'parent_id');
    }


    public function categories()
    {
        return $this->belongsToMany(config('administrable-shop.models.category'), 'shop_products_categories');
    }


    public function brand()
    {
        return $this->belongsTo(config('administrable-shop.models.brand'));
    }
    ...
    public function saveAttributes(string $attributes): void
    {
        ...
        foreach ($attributes as $attr) {
            ...
            if ($attribute_id = Arr::get($attr, 'id')) {
                $attribute = config('administrable-shop.models.attribute')::find($attribute_id);
            } else {
                $attribute = config('administrable-shop.models.attribute')::create(['name' => Arr::get($attr, 'name')]);
            }
    ...
    public function createVariation(array $variation): self
    {
        return $this->children()->create(array_merge($variation, [
            ...
            'attribute_id'       => config('administrable-shop.models.attribute')::findByName(Arr::get($variation, 'attribute'))->first()?->getKey(),
            'term_id'            => config('administrable-shop.models.attributeterm')::findByName(Arr::get($variation, 'term'))->first()?->getKey(),
        ]));
    }
